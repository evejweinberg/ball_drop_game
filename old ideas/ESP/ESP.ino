...
//for maker 1000 we'd have to chage the line above
#include "settings.h"
...
void loop() {
  ...
  // did any data come through the data socket?
  //comes in through the network
  while (socket.available()) {
    Serial.print("Got something");
    String input = socket.readString();
    //print it back to the controller out the serial port
    Serial.print(input);
  }

  // Read the reply from serial port and print them to server:
  //if it comes from serial port, go the other way
...
//when you call this it tries to connect to the server.
//separates out the physical control from code
